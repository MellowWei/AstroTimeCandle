 ...
 * @version 1.1.0-Final-Singularity
...
export default class AstroAwakeEngine {
    ...
    init() {
        ...
        // 2. 音频采样接入：winter_sun.m4a
        this.setupAudio();

        // 3. 核心：Astro Time Candle
        this.createCandle();
    ...
    animate() {
        ...
        // 音频共振：让书本雾蓝随 winter_sun 呼吸
        if (this.analyser) {
            const freq = this.analyser.getAverageFrequency();
            this.candleMat.emissiveIntensity = 0.1 + (freq / 255) * 1.5;
            this.particles.position.y = Math.sin(time) * (freq / 100);
        }
